...
با دکمهٔ `🗂 نشست‌ها` می‌توانی نشست‌ها را ببینی، بینشان جابه‌جا شوی، نشست جدید بسازی یا اجرای یک نشست را متوقف کنی. نام هر نشست **خودکار از روی آخرین موضوع گفتگو** ساخته می‌شود؛ با `✏️` نام دستی بگذار و با `🗑` نشست را ببند. اجرای هر نشست مستقل است؛ ساخت نشست جدید و جابه‌جایی، کاری به اجراهای در جریان ندارد (تا ۶ اجرای هم‌زمان برای هر کاربر).
