...
با دکمهٔ `🗂 نشست‌ها` می‌توانی نشست‌ها را ببینی، بینشان جابه‌جا شوی، نشست جدید بسازی یا اجرای یک نشست را متوقف کنی. نام پیش‌فرض نشست‌ها **شماره است (۱، ۲، ۳…)**؛ با `✏️` می‌توانی نام دلخواه بگذاری (تا زمانی که نشست حذف شود، باقی می‌ماند) و با `🗑` نشست را ببندی. وقتی نشستی را باز می‌کنی (`/use` یا دکمهٔ `➡️`)، فهرست تمام موضوعاتی که تا آن لحظه در همان نشست مطرح شده‌اند، مورد به مورد نمایش داده می‌شود. اجرای هر نشست مستقل است؛ ساخت نشست جدید و جابه‌جایی، کاری به اجراهای در جریان ندارد (تا ۶ اجرای هم‌زمان برای هر کاربر).
