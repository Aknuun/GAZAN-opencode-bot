...
- **⚙️ تنظیمات داخل ربات**:
  ...
  - انتخاب agent
- **⏰ هشدار ساعت پیک (گران)**: قبل از هر اجرا اگر در بازهٔ پیکِ پروایدر باشی، با دکمه تأیید می‌پرسد «ادامه بدهم؟»؛ در پایان هر چت هم وضعیت پیک (🟢 خارج / 🔴 داخل) را نشان می‌دهد. هنگام افزودن کلید API، فعال‌سازی یادآوری پیک همان پروایدر پیشنهاد می‌شود.
...
> در **پایان هر چت**، زیر پیام خلاصه یک دکمهٔ `🗑 حذف این نشست` می‌آید که تا ۱۰ ثانیه فعال است؛ با زدنش نشست همان‌جا حذف می‌شود (روی سرور opencode هم).
